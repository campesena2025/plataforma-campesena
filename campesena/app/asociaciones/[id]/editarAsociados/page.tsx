...
const Page = () => {
  ...
  const [initialAssociates, setInitialAssociates] = useState<Participante[] | null>(null);
    ...
    <Card className="rounded-lg w-full max-w-7xl mx-auto mt-6">
      <CardHeader>
        <div className="flex flex-col">
          ...
          {nombreAsociacion && <p className="text-lg text-default-500">{nombreAsociacion}</p>}
        </div>
      </CardHeader>
      <CardBody>
        {initialAssociates ? (
          <AsociadosTable asociacion={idAsociacion} initialAssociates={initialAssociates} />
        ) : (
