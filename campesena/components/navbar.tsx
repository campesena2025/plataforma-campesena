...
export const NavbarApp = () => {
  ...
  useEffect(() => {
    const statusData = loading ? 'loading' : user ? 'authenticated' : 'unauthenticated';
  ...
  return (
    <Navbar isBordered={true} isMenuOpen={isMenuOpen} maxWidth="xl" position="sticky" onMenuOpenChange={setIsMenuOpen}>
      <NavbarContent>
      ...
      <NavbarContent className="basis-1/5 sm:basis-full" justify="start">
        <ul className="hidden lg:flex gap-4 justify-start ml-2">
          ...
          {status === 'loading' ? (
            ...
            <NavbarItem>
              <Button color="success" variant="flat" onClick={() => router.push('/login')}>
                Iniciar sesión
              </Button>
            </NavbarItem>
          ) : (
      ...
      <NavbarMenu>
        <div className="mx-4 mt-2 flex flex-col gap-2">
          ...
          {status === 'loading' ? (
            ...
            <Button color="success" variant="flat" onPress={() => router.push('/login')}>
              Iniciar sesión
            </Button>
          )}
        </div>
